...
const get_items=async (order_id, items)=>{ 
  ...
  const get_item = async(i) => {
    if(i==null) return   
    let business_pf=await get_commission_single(i.business_id, 'platform_fee_sale', 'business')     
    //let business_pf=12
    let  d={order_id, product_id:i.id, title:i.title, price:i.price, final_price_after_discount:i.final_price_after_discount, price_after_discount:i.price_after_discount, discount_value:i.discount_value, discount:i.discount, qty:i.qty, business_id:i.business_id,  platform_fee:{business:business_pf}}
    //get_items_.push( d);
    return d
  }
...
export default async function  process(props) { 
  ...
  if(action=="create_temp_order"){ 
    ...
    let shipping2 = await supabase
      .from('order_shipping')
      .insert([
        {order_id, status:'pending', method:shipping_method, tracking_code:'xsd123fd' },
      ]) 
    
    let payment = await supabase
      .from('order_payment')
      .insert([
        {order_id, status:'pending', mode:payment_method, transaction_id:'xsd123fd', meta:props, platform_fee:{influencer:platform_fee}},
      ]) 
    ...
    data=order.data[0]
    if(data.items) data.items=await productImages(data.items)
