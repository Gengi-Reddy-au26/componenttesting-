...
const com = ({id, ...props}) => {   
  ...
  const getData3 = async (collab_id) => {     
    ...
    const collab = await axios.post(process.env.API+'/api/business/collab/'+id, {});    
    if(!collab_id) {
  ...
  const handler = async (i) =>{  
    ...
    const res = await axios.post(process.env.API+'/api/business/collab/action', {business_id:user.id, ...i});
    const data = res.data;
