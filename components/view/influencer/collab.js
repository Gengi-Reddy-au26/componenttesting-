...
const com = ({id, ...props}) => {   
  ...
  const getData3 = async (collab_id) => {       
    ...
    const collab = await axios.post(process.env.API+'/api/influencer/collab/'+id, {});
  ...
  const handler = async (i) =>{  
    ...
    const res = await axios.post(process.env.API+'/api/influencer/collab/action', {influencer_id:user.id, ...i});
    const data = res.data;
