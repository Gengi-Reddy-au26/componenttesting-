...
const com = ({id, ...props}) => { 
 ...
 // const args2={}  
  ...
  const payment_result = async (action, {result, meta}) =>{    
    const res = await axios.post(process.env.API+'/api/com/membership/action', {action, user_id:user.id, user_type:user.userType, plan:meta.name, duration:meta.duration, level:meta.level, meta:meta.meta});
    const data = res.data;
  ...
  if(args.userType=='business'){
    view_=<BusinessSubscription id={id} membership={membership} {...args} isPW={isPW}></BusinessSubscription>
  }else if(args.userType=='influencer'){
    view_=<InfluencerSubscription id={id} membership={membership} {...args}  isPW={isPW}></InfluencerSubscription>
  }  
  return (
    <>
      {view_}      
