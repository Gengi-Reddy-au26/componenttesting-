...
const com = ({items, isPW, item, tresholds, handler, ikHandler, collaborateHandler, refreshHandler,  messageHandler, acceptHandler, initPaymentPaidHandler, paidHandler, breakdownHandler, performanceHandler, ...props}) => {
  ...
  if(item){    
    ...
    const initPaymentPaidHandler2= async ()=>{      
      await initPaymentPaidHandler({action:'init_payment_paid', collab_id:item.id, business_id:item.business_id, influencer_id:item.influencer_id, amount:item.total_init_payment, sub_total:item.init_fixed_fee, tax:item.init_tax, platform_fee:item.init_platform_fee})
      refreshHandler({collab_id:item.id}) 
    }
    const paidHandler2= async ()=>{
      await paidHandler({action:'paid', collab_id:item.id, business_id:item.business_id, influencer_id:item.influencer_id, amount:item.total_balance_fee, sub_total:item.balance_fee, tax:item.init_tax, platform_fee:item.platform_fee})
      refreshHandler({collab_id:item.id})  
    }
    ...
    top_=(
      <div className={s.top+' flex items-center  animate__animated animate__fadeIn animate__delay-1s'}> 
        <div className={' flex-grow flex items-center'}>           
          <UserProfile/>
          <div  className={s.fixed_fee}>Fixed fee : Rs {item.init_fixed_fee} </div>
          <div  className={s.upto}>Can go upto : Rs {item.can_go_upto}</div>
        </div>
