...
import {tag, WeightSlider} from '../_blocks/ui';
...
const com = (props) => {
 ...
 const [isV, setV] = useState({
  ...
  const saveHandler2= async (close)=>{
    ...
    const savedProduct = await handler({action, business_id:user.id, ...isData, extra:isExtra, campaign:isCampaign})
  ...
  useEffect(()=>{
    ...
    if(isData.discount){
      discountSet=get_discount({discount:isData.discount})
    }
    setData((prev)=> ({...prev, ...discountSet})) 
  }, [isData.discount])
  ...
  const details = () =>{
    let available_units = (isData.meta && isData.meta.available_units) ? isData.meta.available_units : 0
    return(
      <div>
        {isData.id && 
        ...
        <div className={s.info}>
          <div className="flex items-start">
            ...
            <div className={s.input+" w-3/12 mr-2"}>
              ...
              <Input placeholder="enter available unit. e.g. 10" name="available_units" value={available_units} changeHandler={(v, e, n)=>{changeMetaHandler(v, n)}}/> 
            </div>
          </div>
    ...
    content= (
      <div>
        <div className={s.header}>
          <div className={"flex-grow"}><h4>{isData.title} </h4> {isData.status!='published' && tag({label:isData.status, color:'red'})}</div>
          ...
          <Button type="action2" clickHandler={()=>saveHandler2()}>Save</Button>
          {isData.id && <Button className={"ml-2"} type="action2" clickHandler={()=>saveHandler2("close")}>Save & Close</Button>}
        </div>
