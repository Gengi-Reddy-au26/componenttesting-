...
const get_track=(i)=>{
  ...
  return(
    <div className={s.track} style={{backgroundImage:'linear-gradient('+gradient_+')'}}>
      {dot({level:CONSTANTS.order, status:i.status})}
      {dot({level:CONSTANTS.awaiting_processing, status:i.status})}
      {dot({level:CONSTANTS.processing, status:i.status})}
      {dot({level:CONSTANTS.payment_received, status:i.status})}
      {dot({level:CONSTANTS.shipped, status:i.status})}
