    ...
    <div className={"mx-auto"} style={{maxWidth:"1240px"}}>
      <div className={"flex flex-wrap mb-8"}>
        <div className={"w-full md:w-4/12 pr-2" }>
          ...
          <Select name="i_gender" value={inData2.meta.i_gender} isMulti={false} options={genderOptions} placeholder="select gender"  changeHandler={(v, n)=>{metaChangeHandler(v, n)}}/>          
        </div>
        <div className={"w-full md:w-4/12 pr-2" }>
          <div className={s.label}>Followers</div>
          <Select name="i_followers" value={inData2.meta.i_followers} isMulti={false} options={followersOptions} placeholder="select followers"  changeHandler={(v, n)=>{metaChangeHandler(v, n)}}/>          
        </div>
        <div className={"w-full md:w-4/12" }>
          <div className={s.label}>Industry</div>
          <Select name="i_industry" value={inData2.meta.i_industry} isMulti options={industryOptions} placeholder="select industry"  changeHandler={(v, n)=>{metaChangeHandler(v, n)}}/>          
        </div>
      </div>  
