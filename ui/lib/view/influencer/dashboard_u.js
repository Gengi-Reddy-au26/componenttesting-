...
const com = (props) => {  
  ...
  const recent_collabs=()=>{
    let items=c.map(({id, status, business, product})=>{
      return {title:business.name, sub_title:product.title, image:business.image, image2:product.image, type:'collab', status,  url:'/app/collab/'+id, }
    })
    return(<RecentItems {...{
      sub_title:"Recent",
  ...
  const recent_order_items=()=>{
    let items=o.map(({item, order, business, product})=>{
      return {title:product.title, sub_title:business.name, image:product.image, type:'order', status:item.status, url:'/app/order/'+order.id, info:item.price+' Rs.'}
    })
    return(<RecentItems {...{
      sub_title:"Recent",
      title:"Order Items",
