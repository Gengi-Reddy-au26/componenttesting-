...
const com = ({index, item, userType, handler, ikHandler, user, influencerAcceptHandler, influencerRejectHandler, paidHandler, initPaymentPaidHandler, priceHandler, performanceHandler, i}) => {
  ...
  const BusinessSign=()=>{
    return(
      <div  className={"mt-8"}>        
        ...
        <div className={s.from}>Team {item.business.name}</div>        
      </div>
    )
  }
  const BusinessHead=()=>{
 ...
 //console.log("collab--", item)
  ...
  const Collaborate=()=>{ 
    ...
    return(
      <div>
        ...
        <div className={"ml-8"}>
          <div>Initial Payment: Rs {item.init_fixed_fee}</div>
  ...
  const InitPayment=()=>{    
    return(
      <div>          
        ...
        <div className={"ml-8"}>       
          <p>Initial Payable Amount - Rs {item.init_fixed_fee}</p>
          {userType=="business" && 
          <div>
  ...
  const shareLink=()=>{
    ...
    let {product_url='', short_code=''} = share_url
  ...
  const PerformanceVideo=()=>{    
    let {performance_video_id, performance_video} = item.meta
    let level=collab_levels[item.status]
    ...
    return(
      <div>        
        ...
        {performance_video_id!==false && play_button_}  
